        ...
        {links.map(
          link =>
            link.show && (
              <li
                key={link.id}
                className="block ml-4 text-black dark:text-white nav"
              >
...
export default function Header ({ navBarTitle, fullWidth }) {
  ...
  return (
    <>
      <div className="observer-element h-4 md:h-4" ref={sentinelRef}></div>
      <div
        className={`sticky-nav group m-auto w-full h-6 flex flex-row justify-between items-center mb-2 md:mb-6 py-6 bg-opacity-60 ${
...
const HeaderName = forwardRef(function HeaderName ({ siteTitle, siteDescription, postTitle, onClick }, ref) {
  return (
    <p
      ref={ref}
      className="header-name ml-2 text-lg md:text-xl text-black dark:text-white capture-pointer-events grid-rows-1 grid-cols-1 items-center"
      onClick={onClick}
    >
